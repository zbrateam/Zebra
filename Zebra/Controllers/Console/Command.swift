...
class Command {
	...
	@discardableResult
	@objc class func execute(_ command: String, arguments: [String]?, asRoot: Bool = false) async throws -> String? {
		...
		let task = Command(command: command,
											 ...
											 asRoot: asRoot,
											 delegate: nil)
		return try await task.execute() == 0 ? task.output : nil
	}
		...
		return try await withCheckedThrowingContinuation { result in
			Task(priority: priority) {
				...
				if spawnResult < 0 {
					...
					result.resume(throwing: ExecuteError.spawnFailed(code: spawnResult))
					return
				}
				...
				result.resume(returning: WIFEXITED(status) ? WEXITSTATUS(status) : status)
			}
		}
	}
